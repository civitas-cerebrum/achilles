...
| `atelier/` | **harness-atelier** — context-flow observability, designed as a standalone repo-shaped package (own README/package.json/tests, severable). Visualizes agent/subagent context use, transfers, and leaks from a five-event JSONL telemetry log; `npm run atelier` renders the report, a Stop hook keeps it fresh, `--baseline` gives a CI-assertable regression diff. See [`atelier/README.md`](atelier/README.md) | you (the report), CI (`--json`) |
