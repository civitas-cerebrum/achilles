...
- **It binds when the protocol activates** — an achilles skill is invoked, `/<skill>` is typed, or a role-prefixed subagent is dispatched. From that moment the **main session is the `orchestrator`** role: the table grants it `tests/**`, `.gitignore` and `.achilles/**` to write, the suite and `git status/log/diff/add/commit` to run, the achilles skills to invoke, and every subagent role to dispatch — and it may not read `src/**` or `.env`, nor write `playwright.config.ts` or `package.json`. A role that runs the runner must not author the runner's config, so those files — and the rest of the Phase 1–2 scaffold (fixtures, `playwright.setup.ts`, the page repository, `app-context.md`) — belong to a write-only **`scaffolder`** role with no shell and no dispatch, which the orchestrator dispatches and then verifies with `npx playwright test --list`. Subagents bind to their own roles (`scaffolder`, `test-composer`, `in-flight-composer`, `workflow-reviewer`, `phase-validator`, `process-validator`, `batch-reviewer`, `perf-reviewer`, `selector-diff-validator`): approvers write only their ledger and have no shell; composers write only `tests/e2e/**` and declare `write.codeImports` = `@playwright/test`, `@civitas-cerebrum/element-interactions`, so a spec importing anything else by package name (`fs`, say) is refused. Every deny names the role's mandate and the sanctioned alternative — `kernel-mandate explain --role orchestrator --tool Write --path playwright.config.ts` shows the verdict, and names the scaffolder.
- **Dispatches carry the role.** The kernel resolves a dispatch's target role from its description prefix and binds the child through a tag in the prompt, so every achilles dispatch is `description: <role>-<slug>: <task>`, `subagent_type: <role>`, and a prompt whose first line is `<<kernel-mandate-role: <role>#<nonce>>>` with a fresh nonce per dispatch. The composer role is `test-composer`, so the composer dispatch is `test-composer-j-<slug>:`; the pre-kernel `composer-j-<slug>:` names no role and is refused. The full grammar is in `skills/onboarding/SKILL.md` §"Dispatch grammar".
