...
For the canonical return shape every subagent uses, see `../achilles-protocol/references/subagent-return-schema.md`.
...
5. Return a structured discovery report only — no pasted test source, no DOM snapshots, no CLI transcripts. Returns follow the canonical return schema in [`../achilles-protocol/references/subagent-return-schema.md`](../../achilles-protocol/references/subagent-return-schema.md); the dispatch brief includes a pointer to the file rather than re-pasting the schema.
...
2. Additionally receive: the pass number (4 or 5), the ledger file path (`tests/e2e/docs/adversarial-findings.md`), the lockfile path (`tests/e2e/docs/.adversarial-findings.lock`), and a pointer to the canonical schema at `skills/achilles-protocol/references/subagent-return-schema.md`.
