...
1. **Open the dedicated `playwright-cli` session.** Open your assigned session at the start: `npx playwright-cli -s=reviewer-j-<slug>-<pass>-c<C> open --browser=chromium <baseURL>`. Sessions are OS-isolated by construction — there is no isolation-prerequisite check (see [`../../element-interactions/references/playwright-cli-protocol.md`](../../element-interactions/references/playwright-cli-protocol.md) §1). Close it at the end with `npx playwright-cli -s=<your-slug> close`.
