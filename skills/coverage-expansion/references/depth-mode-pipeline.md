...
2. **Recompute priority ordering.** Honour the map's priorities, but if a journey's `Test expectations` or pages touched have changed since the last pass, adjust position. Within a tier, journeys whose map block is `risk: elevated` (2+ `Risk factors:` tags) dispatch ahead of `risk: baseline` siblings — risk orders *within* a tier; it never reorders across tiers.
...
- **No elevated-risk journeys.** A journey whose map block carries 2+ `Risk factors:` tags (`risk: elevated` per `journey-mapping/references/phases.md` §"Defect-likelihood risk factors") is dispatched per-journey, not in a group — its failure surface is exactly what grouped attention-rationing misses. Journeys without the field default to `risk: baseline` and group normally.
