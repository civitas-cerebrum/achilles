...
**Role-prefix.** The dispatch description is `[group] test-composer-j-<a>,test-composer-j-<b>,...:`. Cap-7 rule, enforced by methodology (the comma count in the description tells you whether you're at the cap). The harness dispatch-guard hook that previously enforced the cap mechanically was retired in the 0.3.6 cleanup; the rule still applies. Items must be role-explicit `composer-` slugs (compositional passes). The same `[group]` marker is also accepted for `probe-j-` items by `bug-discovery` Phase 6 — see `bug-discovery/SKILL.md`. Returns are per-journey concatenated under one Agent return.
