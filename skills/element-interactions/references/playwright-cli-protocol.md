...
`@playwright/cli` is a **hard `dependencies` entry** of `@civitas-cerebrum/achilles` (this package — not `element-interactions`, which does not depend on it). After `npm install @civitas-cerebrum/achilles`, the CLI binary is reachable via `npx playwright-cli ...` immediately — no extra `npm install -D @playwright/cli` step. The postinstall script confirms reachability and prints the version.
...
- **`@playwright/cli` is alpha (v0.1.x as of 2026-05-01).** It is shipped as a hard `dependencies` entry of `@civitas-cerebrum/achilles` (this package — not `element-interactions`, which does not depend on it), pinned to a specific patch version (currently `0.1.17`). When the CLI ships a breaking change, this package's pin is bumped on the same release that absorbs the change — consumers never have to think about CLI versions. The pin tightens (caret → exact) precisely *because* it's alpha; let it become `^X.Y.Z` only after 1.0.
