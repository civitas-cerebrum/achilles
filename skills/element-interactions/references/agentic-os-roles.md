...
| `app-fetch` | Any body-returning `curl`/`wget` into the executing window (host-agnostic — distinguishing app-under-test from a package registry by host is unreliable, so all response bodies are treated as payload). Bounded probes stay allowed: `curl -I`/`--head`, body discarded to `/dev/null`; `wget`-to-file (disk, not window) is not this class. | **orchestrator** (response bodies are payload) |
...
The Bash/Read heuristics are detection layers, not proofs — the OS-user layer (below) is the unbypassable enforcement. The detectors are nonetheless kept honest: dump detection covers `cat`/`head`/`grep`-content/`awk`/`xargs`-chains and tolerates `command`/`env`/`sudo`/subshell wrappers; the browser and app-fetch detectors share the same command-word anchor; and every orchestrator class is enforced by consulting the declared denied-set (`role_denied_classes orchestrator`), so the map and the guard cannot silently fork.
