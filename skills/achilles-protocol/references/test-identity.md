...
- **Shared tag sites re-scope first.** Both paths edit the tag ONLY at a site that scopes solely the anomalous test (its own title or `{ tag: … }` option). When `@known-defect` sits on an enclosing describe title — or any tag site shared with sibling tests that are still red — do not strip or replace it there: move the tag down onto the individual tests first, so each still-red sibling keeps `@known-defect` and its defect pointer, and the anomalous test alone gets its resolution. Stripping a shared site silently turns the siblings' filed reds into ordinary failures the next session diagnoses from scratch; retagging a shared site marks deterministic filed defects `@flaky`.
