...
| **h0. Known defect — no heal, no rerun** | **Report** | The test (or its describe) carries `@known-defect` and the failure signature matches the filed defect | Terminal on sight: do not reproduce, do not experiment, do not heal, do not rerun. Record it as a known defect in the summary and move on. A *different* error behind the tag is a second, unfiled problem — diagnose that one normally. A `@known-defect` test that passes means the defect is fixed: report it and drop the tag. Contract: [`test-identity.md`](../achilles-protocol/references/test-identity.md) §2 |
