...
The kernel mandate is a role-based operating layer for multi-agent harnesses.
...
| Piece | File | Job |
|---|---|---|
| Manifest schema | [`schemas/kernel-mandate.schema.json`](../../../schemas/kernel-mandate.schema.json) | Validates a project's role manifest |
| Kernel hook | [`hooks/kernel-mandate-role-gate.sh`](../../../hooks/kernel-mandate-role-gate.sh) | PreToolUse `.*` — resolves the caller's role, enforces its grants |
| Onboarding skill | [`skills/mandate-designer/SKILL.md`](../SKILL.md) | Interviews the user, writes + validates the manifest |

The standalone package adds a CLI (`npx kernel-mandate init` to register the
hook project-locally or globally, `npx kernel-mandate validate` to check a
manifest including the cross-references JSON Schema cannot express).

## The manifest

Lives at `<repo-root>/.claude/kernel-mandate.json` in the *consumer* project
(override with `KERNEL_MANDATE_MANIFEST` for tests). **Presence of the
...
that did not opt in. `KERNEL_MANDATE=0` in the operator's shell environment
...
Worked manifests: [examples/qa-pipeline.kernel-mandate.json](../examples/qa-pipeline.kernel-mandate.json)
...
`<repo-root>/.claude/kernel-mandate.state/agents/<agent_id>` so the ladder
runs once per agent:
...
5. **Registry claim** — the dispatch registry
   (`.claude/kernel-mandate.state/dispatch-registry.json`, written at
...
sessions, whereas the kernel mandate scopes itself by manifest presence in
...
`mandate-designer` is intentionally absent from achilles' activation
...
- State dir: `<repo-root>/.claude/kernel-mandate.state/` —
...
- Both the manifest and the state dir are covered by the built-in
  ...
  operator-run design session (`KERNEL_MANDATE=0`) or a hand edit.
- **The write-verb table is a floor, not a boundary.** `uniq` writes its
  ...
  the enumeration stays and `kernel-mandate validate` reports which command
...
- **The axis covers every path that could BECOME law, not just the one
  ...
  `.claude/kernel-mandate.json`, so a role that could author one inside its
...
specs from the role that RUNS them.** `kernel-mandate validate` says so for
...
that runs it*, so no single mandate spans both; `kernel-mandate validate`
warns when one role holds both, and the designer skill teaches the split.

Where the split is impractical, `kernel-mandate run` is the second half of
the answer:

```bash
kernel-mandate run --role composer -- npm test
```
...
(`^kernel-mandate run --role composer -- npm test`), so the role cannot
...
`kernel-mandate run --role composer -- cat .env` would apply no containment
...
(`kernel-mandate run --role r -- node <script>`) avoids that.
...
- **`bash.unrestricted`** is a deliberate hole for a trusted role, and
  ...
  `kernel-mandate explain` previews a verdict, `kernel-mandate doctor` turns the
