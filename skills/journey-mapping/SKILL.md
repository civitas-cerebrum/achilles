...
- `tests/e2e/docs/journey-map-coverage.md` — the journey→spec coverage
  matrix. This is a **separate, required deliverable** (onboarding Phase 4,
  the `workflow-reviewer-phase4` checklist, and the resume logic all read
  it; `journey-map-sentinel-gate.sh` gates its write). Author it right after
  `journey-map.md` — the sentinel gate allows the coverage file only once a
  sentinel-bearing `journey-map.md` exists. One row per journey in the map,
  in the same priority order:

  ```markdown
  # Journey → Spec Coverage
  <!-- derived from journey-map.md; regenerated by coverage-expansion each pass -->

  | Journey | Priority | Spec | Status |
  |---|---|---|---|
  | j-sign-in | P0 | tests/e2e/sign-in.spec.ts | covered |
  | j-checkout | P0 | <missing> | missing |
  | j-wishlist | P2 | <missing> | missing |
  ```

  At Phase-4 authoring time every journey is `<missing>` EXCEPT those already
  covered by a Phase-3 happy-path spec (cross-reference the specs on disk).
  Coverage-expansion updates the matrix as it lands specs; the `<missing>`
  rows are its worklist and the reviewer's P0/P1-close-first gate reads them.
