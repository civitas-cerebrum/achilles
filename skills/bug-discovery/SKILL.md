...
- `@playwright/cli` is reachable (`npx --no-install playwright-cli --version` exits 0). Since the CLI ships as a hard dependency of `@civitas-cerebrum/achilles` (not `element-interactions`), this almost always passes; a non-zero exit means a corrupted install and the fix is `npm install`, not a separate dep add. The browser binary may still need a one-shot fetch — `npx playwright-cli install-browser chromium`.
