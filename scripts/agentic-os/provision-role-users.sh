...
#
# Security invariant (do NOT add achl-* users as sudoers): only
# ${SESSION_USER} appears on the left of a rule here. The achl-* role
# users are deliberately NOT granted sudo. This is what makes
# hooks/agentic-user-exec.sh's always-wrap safe: a subagent that
# hand-writes its own \`sudo -u achl-<other>\` is nested inside the
# wrapper and runs AS an achl-* role user, who cannot sudo — the inner
# escalation fails closed. Granting any achl-* user sudo here would
# reopen that hole.
