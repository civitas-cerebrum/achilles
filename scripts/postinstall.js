...
// __dirname is `<package>/scripts/` — `packageDir` walks one level up
...
function installCivitasSkills() {
  ...
  try {
    ...
    if (installedSkills.size > 0) {
      console.log(`[@civitas-cerebrum/achilles] ✔ ${installedSkills.size} skill${installedSkills.size > 1 ? 's' : ''} installed to ${destinations.length} locations — restart Claude Code to pick it up.`);
    } else {
...
const HOOK_MANIFEST = [
  ...
  // Sibling of the above, gating the OTHER half of ticket-driven-testing's
  // contract: item 3, the evidence bundle. Binds per TICKET rather than per
  // session, so a second ticket worked in the same session cannot sign off on
  // the first ticket's evidence — the mechanical half of the skill's §0
...
// The human-readable copy of the same mandate, derived by
// `kernel-mandate doc` and regenerated by npm run sync:kernel-mandate.
// Staged beside the manifest because a project that has an operating
// system imposed on it deserves the page that explains it: the ten roles,
// what each one is REFUSED, where work changes hands, and the review
// loops. The kernel never reads this file — it is for the people.
