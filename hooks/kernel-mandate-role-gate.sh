...
# (.claude/kernel-mandate.json, schema: schemas/kernel-mandate.schema.json),
# resolves WHICH role the calling context is (main session vs dispatched
# subagent — see the resolution ladder in lib/kernel-mandate.sh), and
...
if [ -z "$JQ" ]; then
  ...
  # its own remedy, and can be waved through with KERNEL_MANDATE=0, whereas
...
self_protect_target() {
  ...
  # from cwd and stop at the first `.claude/kernel-mandate.json`, on purpose,
  ...
  # `NORM_MANIFEST` — so `tests/e2e/.claude/kernel-mandate.json` matched
  ...
  # tests/e2e/.claude/kernel-mandate.json` was already refused while the
  ...
  # — discovery only ever looks at `<dir>/.claude/kernel-mandate.json` — but
...
case "$KM_TOOL" in
  ...
  Bash)
    ...
    # .claude` must not escape just because it never says "kernel-mandate".
    ...
    if [ -n "$CMD" ] && printf '%s' "$CMD" | grep -Eqi "$PROT_RE"; then
      ...
      # modify the kernel mandate itself". The verdict was right by accident
      ...
      #     .claude/kernel-mandate.json                     ->  ALLOW
      #
          ...
          if printf '%s' "$__tok" | grep -Eqi "$PROT_RE"; then
            case "$__tok" in
              ...
              *)
                ...
                # `%output{.claude/kernel-mandate.json}PWNED` — where the
...
check_code_capabilities() {
  ...
  # `kernel-mandate run` or splitting authoring from running.
...
$(printf '%s' "$code" | perl -0777 -pe 's{/\*.*?\*/}{ }gs; s{(^|[^:"\x27\\])//[^\n]*}{$1}g;
  ...
  if printf '%s' "$CODE_N" | grep -Eq "${LOAD}[[:space:]]*\([[:space:]]*\"[[:space:]]*(fs|fs/promises|os)[[:space:]]*\"|from[[:space:]]*\"[[:space:]]*(fs|fs/promises)[[:space:]]*\"|^[[:space:]]*import[[:space:]]+(os|shutil|pathlib|io|glob)([[:space:],.]|$)|^[[:space:]]*from[[:space:]]+(os|shutil|pathlib|io|glob)([[:space:].]|$)|(^|[^a-zA-Z_.])open[[:space:]]*\([[:space:]]*[\"'\`]|${FS_METHODS}|readFile[[:space:]]*\(|(^|[^A-Za-z0-9_])Path[[:space:]]*\("; then
    ...
    # profile `kernel-mandate run` installs. It is not process spawning and
  ...
  # The structural answer is in `kernel-mandate validate`, which now names
  # any role that both authors and runs, and in `kernel-mandate run`.
  if [ -z "$CAP_ID" ] \
  ...
  # a floor, not a boundary; the boundary is `kernel-mandate run`.
  if [ -z "$CAP_ID" ]; then
  ...
  # it without either this list or the `kernel-mandate run` wrapper, and the
...
Everything after an unclosed quote reads as string rather than syntax, so the kernel cannot tell which parts are commands and which are text. A shell would refuse this command too. Close the quote and send it again."
  ...
  while IFS= read -r seg; do
    ...
    # unusable.
    # `kernel-mandate run --role X` installs role X's runtime profile. A role
    # invoking it with someone ELSE's name borrows that role's scopes —
    # and a command group written loosely (`^kernel-mandate run\b`) permits
...
NOTE: your command group DOES grant 'git ${GIT_SUB}'. This refusal is a construct deny that overrides it, not a missing grant — so adding another command pattern will not help. Git history is a second copy of the working tree, so an unconstrained 'git ${GIT_SUB}' reads whatever the history holds — including files this role's read scope excludes. Options, narrowest first:
  ...
  3. If reading history wholesale really is this role's job, the operator
    ...
    if [ "$BASH_UNRESTRICTED" != "true" ]; then
      ...
      # `kernel-mandate run` either — npm needs --allow-child-process, and a
...
Your command group DOES grant '${SEG_WORDS[0]}'. This refusal is about WHERE it reads, not whether you may run it: with no path operand it walks the whole project, so it is held to the same scope as naming that directory outright, which is refused for this role too. Name a path inside your read scope instead: $(printf '%s' "$READ_ALLOW" | "$JQ" -r '.[0] // "<a path in scope>"' 2>/dev/null)"
    ...
    # redirect targets, mutate verbs, flag operands. `kernel-mandate import
    ...
    #
    # Round 42 built a bundle with `kernel-mandate export` — which computes a
    ...
    # lists as protected. `kernel-mandate use <name>` is quieter still: its
...
A pattern is applied under the search root, so '..' escapes the role's scope. Narrow the search with the 'path' argument (kept inside your read scope) instead of globbing upward." ;;
    ...
    # root — see kernel_mandate_search_root, which both this arm and the
    # unbound arm call.
    if [ -z "$TARGET" ]; then
      kernel_mandate_search_root
...
when several roles run at once: <<kernel-mandate-role: ${TARGET_ROLE}#a1b2c3>>."
    ...
    # A NEAR MISS IS REFUSED, not waved through. `<<kernel-mandate-role:  judge>>`
