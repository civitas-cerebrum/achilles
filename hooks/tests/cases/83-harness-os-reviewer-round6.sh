...
# `harness-os run` wrapper, and says the screen is advisory until one is
...
# which is why `harness-os run` (a runtime permission profile built from
...
section "harness-os reviewer round-6 regressions"
...
export HARNESS_OS_STATE_DIR="$R6/state"
