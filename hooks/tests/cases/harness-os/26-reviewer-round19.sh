...
section "harness-os reviewer round-19 regressions"
...
printf 'print(1)\n' > "$P/tests/a.py"
printf '# readme\n' > "$P/README.md"
export HARNESS_OS_STATE_DIR="$R19/state"
...
printf 'analyst\n' > "$HARNESS_OS_STATE_DIR/agents/analyst"
bpay() { "$JQ" -nc --arg c "$1" \
...
JSON
printf 'tester\n' > "$HARNESS_OS_STATE_DIR/agents/tester"
printf 'scripter\n' > "$HARNESS_OS_STATE_DIR/agents/scripter"
printf 'plain\n' > "$HARNESS_OS_STATE_DIR/agents/plain"
