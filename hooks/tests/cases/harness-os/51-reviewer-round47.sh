...
#
#   F5  `harness_os_path_in_scope` matched with `grep`, which is
...
section "harness-os reviewer round-47 regressions"
...
export HARNESS_OS_STATE_DIR="$R47/state"
...
test('t', async ({ context }) => { await context.storageState({ indexedDB: Boolean(0), path: '.claude/settings.json' }); });")" \
  "R47 a nested paren before the operand no longer hides the call → DENY" "harness OS itself"
assert_deny "$H" "$(spec "import { test } from '@playwright/test';
const opts = { path: '.claude/harness-os.json' };
test('t', async ({ context }) => { await context.storageState(opts); });")" \
  "R47 nor an options object bound to a variable → DENY" "harness OS itself"
assert_deny "$H" "$(spec "import { test } from '@playwright/test';
