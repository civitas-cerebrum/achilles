...
# "no".
#
#   F1  `harness_os_deny` rendered its JSON with `jq --arg r "$reason"`,
...
assert_eq "$(has "$BIG_REASON" "elided by harness-os")" "yes" \
...
assert_eq "$(has "$SMALL_REASON" "elided by harness-os")" "no" \
...
OUT=$(nojq_run "$GOV" HARNESS_OS_MANIFEST="$P/.claude/harness-os.json")
...
assert_eq "$(nojq_run "$UNGOV" -u HARNESS_OS_MANIFEST | wc -c | tr -d ' ')" "0" \
  "self-probe calibration: jq missing and NO manifest → still a silent allow"
assert_eq "$(nojq_run "$GOV" HARNESS_OS=0 HARNESS_OS_MANIFEST="$P/.claude/harness-os.json" | wc -c | tr -d ' ')" "0" \
...
"$JQ" '.harnessOsVersion = 2' "$P/.claude/harness-os.json" > "$V2/.claude/harness-os.json"
OUT=$(printf '%s' "$("$JQ" -nc '{tool_name:"Bash",tool_input:{command:"cat .env"},cwd:"'"$V2"'",agent_id:"composer"}')" \
  | HARNESS_OS_MANIFEST="$V2/.claude/harness-os.json" bash "$H" 2>/dev/null)
assert_eq "$(printf '%s' "$OUT" | "$JQ" -r '.hookSpecificOutput.permissionDecision // "ALLOW"' 2>/dev/null || echo ALLOW)" \
