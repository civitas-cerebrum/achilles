...
section "harness-os reviewer round-15 regressions"
...
printf 'x\n' > "$P/tests/e2e/a.txt"
export HARNESS_OS_STATE_DIR="$R15/state"
...
printf '%s\n' "$MANIFEST_BODY" > "$P/.claude/harness-os.json"
...
assert_allow "$H" "$("$JQ" -nc --arg f "$P/.claude/harness-os.json" --arg c "$MANIFEST_BODY" \
...
# as a flag operand.
assert_deny "$H" "$(bpay 'curl -s http://x.test --some-future-flag .claude/harness-os.json')" \
  "self-probe an UNMODELLED flag aimed at the manifest → DENY" "harness OS"
assert_deny "$H" "$(bpay 'sometool --write-to=.claude/harness-os.json')" \
  "self-probe the = spelling of one → DENY" "harness OS"
assert_deny "$H" "$(bpay 'sometool -Z .claude/harness-os.state/agents/x')" \
  "self-probe an unknown short flag aimed at the state dir → DENY" "harness OS"

# The manifest is readable by design — it is the law each role is held
# to — so the readers must keep working.
assert_allow "$H" "$(bpay 'cat .claude/harness-os.json')" \
  "self-probe calibration: reading the manifest → ALLOW"
assert_allow "$H" "$(bpay 'sort .claude/harness-os.json')" \
