...
section "harness-os reviewer round-1 regressions"
...
export HARNESS_OS_STATE_DIR="$RR/state"
...
assert_deny "$H" "$(payload tool_name=Bash command='cd . > node_modules/@civitas-cerebrum/harness-os/hooks/harness-os-role-gate.sh' cwd="$P" $I)" \
  "R3 'cd . > <vendored kernel>' — disabling enforcement → DENY" "harness OS itself"
assert_deny "$H" "$(payload tool_name=Write file_path="$P/node_modules/@civitas-cerebrum/harness-os/hooks/harness-os-role-gate.sh" content='#!/bin/bash
exit 0' cwd="$P" $C)" \
  "R3 Write to the vendored kernel hook → DENY (root of trust, wherever installed)" "harness OS itself"
