...
section "harness-os reviewer round-39 regressions"

R39=$(mktemp -d)
P="$R39/proj"
mkdir -p "$P/.claude" "$P/tests/e2e/data" "$P/docs"
printf 'x\n' > "$P/tests/e2e/data/cv.pdf"
export HARNESS_OS_STATE_DIR="$R39/state"
...
assert_deny "$H" "$(wp "$(spec 'await page.screenshot({ path: "../../.claude/harness-os.json" });')")" \
  "R39 aimed at the manifest → DENY" "the harness OS itself"
assert_deny "$H" "$(wp "$(spec 'await page.screenshot({ path: "../../.claude/settings.json" });')")" \
  "R39 aimed at the hook registration → DENY" "the harness OS itself"
