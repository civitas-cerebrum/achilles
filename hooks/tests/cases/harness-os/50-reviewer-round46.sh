...
# `harness-os validate` now names command words a role's groups admit
# that the kernel classifies as neither provably read-only nor a
# modelled write verb.

H="$HOOK_DIR/harness-os-role-gate.sh"

section "harness-os reviewer round-46 regressions"
...
export HARNESS_OS_STATE_DIR="$R46/state"
