...
section "harness-os reviewer round-29 regressions"

R29=$(mktemp -d)
P="$R29/proj"
mkdir -p "$P/.claude" "$P/docs" "$P/tests"
export HARNESS_OS_STATE_DIR="$R29/state"
...
reset_state() { rm -rf "$HARNESS_OS_STATE_DIR"; mkdir -p "$HARNESS_OS_STATE_DIR/agents"
  printf 'orchestrator\n' > "$HARNESS_OS_STATE_DIR/agents/orch"
  printf 'runner\n'       > "$HARNESS_OS_STATE_DIR/agents/run1"; }
...
assert_deny "$H" "$(ap run1 tuidA2 'composer-x: write' '<<harness-os-role: composer>>
write it')" \
...
# still a boundary.
assert_allow "$H" "$(ap orch tuidB 'composer-x: write the spec' '<<harness-os-role: composer>>
write it')" \
  "A calibration: a declared dispatch still works → ALLOW"
assert_deny "$H" "$(ap orch tuidC 'judge-x: grade it' '<<harness-os-role: judge>>
grade it')" \
...
write_manifest off; reset_state
printf 'orchestrator\n' > "$HARNESS_OS_STATE_DIR/agents/orch2"
"$JQ" -nc '{"tuidJ":{"role":"judge","ts":9999999999}}' > "$HARNESS_OS_STATE_DIR/dispatch-registry.json"
