...
# `harness_os_url_authority`, so every one of them inherited the
...
section "harness-os reviewer round-35 regressions"

R35=$(mktemp -d)
P="$R35/proj"
mkdir -p "$P/.claude" "$P/docs"
printf 'ac\n' > "$P/docs/ac.md"
export HARNESS_OS_STATE_DIR="$R35/state"
...
JSON
printf 'scoped\n' > "$HARNESS_OS_STATE_DIR/agents/scoped"
