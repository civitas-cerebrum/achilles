...
section "harness-os reviewer round-52 regressions"

R52=$(mktemp -d)
P="$R52/proj"
mkdir -p "$P/.claude" "$P/tests/e2e" "$P/docs"
printf '{}\n' > "$P/docs/e2e-ledger.json"
export HARNESS_OS_STATE_DIR="$R52/state"
...
assert_deny "$H" "$(b 'curl -s http://localhost:4173/ -w "%output{.claude/harness-os.json}PWNED"')" \
  "R52 %output{} aimed at the manifest, from a role with NO write grants → DENY" "harness OS itself"
assert_deny "$H" "$(b 'curl -s http://localhost:4173/ --write-out "%output{.claude/settings.json}{}"')" \
  "R52 ...the long flag spelling, at the hook registration → DENY" "harness OS itself"
...
assert_deny "$H" "$(b 'somefilter --opt "prefix:.claude/harness-os.json"')" \
  "R52 a protected path embedded in a larger operand → DENY" "harness OS itself"
...
assert_allow "$H" "$(b 'cat .claude/harness-os.json')" \
