...
section "harness-os identity / dispatch nonce"
...
export HARNESS_OS_STATE_DIR="$ID/state"
...
GOOD_NONCE=$'<<harness-os-role: inspector#n1a2b3>>\nProbe the form.'
...
if "$JQ" -e '."nonce:n1a2b3".role == "inspector"' "$HARNESS_OS_STATE_DIR/dispatch-registry.json" >/dev/null 2>&1; then
...
printf '%s\n' '{"type":"user","content":"<<harness-os-role: inspector#n1a2b3>> probe src. (a mention of <<harness-os-role: reviewer#zzzz99>> that is not a real dispatch)"}' > "$NOISE_TRANSCRIPT"
...
printf '%s\n' '{"type":"user","content":"<<harness-os-role: reviewer>> review the deliverable"}' > "$CLEAN_TRANSCRIPT"
...
MIXED=$'<<harness-os-role: inspector#aaaa11>>\nand <<harness-os-role: reviewer#bbbb22>> too'
