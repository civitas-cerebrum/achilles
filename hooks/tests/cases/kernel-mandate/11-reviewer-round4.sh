...
section "kernel-mandate reviewer round-4 regressions"
...
export KERNEL_MANDATE_STATE_DIR="$R4/state"
...
assert_deny "$H" "$(payload tool_name=Bash command='env echo x>.claude/settings.json' cwd="$P" $C)" \
  "N1 overwriting the hook REGISTRATION → DENY" "kernel mandate itself"
assert_deny "$H" "$(payload tool_name=Bash command='env echo disabled>node_modules/@civitas-cerebrum/kernel-mandate/hooks/kernel-mandate-role-gate.sh' cwd="$P" $C)" \
  "N1 overwriting the vendored KERNEL → DENY" "may not author"
assert_deny "$H" "$(payload tool_name=Bash command='env echo x>.claude/kernel-mandate.json' cwd="$P" $C)" \
  "N1 overwriting the MANIFEST → DENY" "kernel mandate itself"
