...
section "kernel-mandate reviewer round-48 regressions"
...
export KERNEL_MANDATE_STATE_DIR="$R48/state"
...
printf 'reviewer\n' > "$KERNEL_MANDATE_STATE_DIR/agents/r"
...
assert_deny "$H" "$(spec "test.use({ recordVideo: { dir: '.claude/hooks' } });")" \
  "R48 ...and one aimed at a protected child is self-protection → DENY" "kernel mandate itself"
...
printf 'author-only\n' > "$KERNEL_MANDATE_STATE_DIR/agents/a"
"$JQ" '.roles["author-only"] = {description:"Authors specs and cannot run them.",tools:{allow:["Write"]},read:{allow:["tests/**"]},write:{allow:["tests/e2e/**"],codeImports:["@playwright/test"]}}' \
  "$P/.claude/kernel-mandate.json" > "$P/.claude/tmp.json" && mv "$P/.claude/tmp.json" "$P/.claude/kernel-mandate.json"
