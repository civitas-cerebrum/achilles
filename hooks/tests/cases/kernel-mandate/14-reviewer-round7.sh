...
#
#   F1  `kernel-mandate run` — the layer round 6 elevated to "the boundary" —
...
section "kernel-mandate reviewer round-7 regressions"

R7=$(mktemp -d)
P="$R7/proj"
mkdir -p "$P/.claude" "$P/tests/e2e"
printf 'SMTP_PASSWORD=hunter2\n' > "$P/.env"
export KERNEL_MANDATE_STATE_DIR="$R7/state"
...
# `kernel-mandate run --role X` installs X's path scopes as the process's
...
# (`^kernel-mandate run\b`) permits exactly that, and that is a shape an
...
JSON
KERNEL_MANDATE_MANIFEST="$R7B/.claude/kernel-mandate.json" assert_deny "$H" \
  "$(payload tool_name=Bash command='kernel-mandate run --role admin -- node x.js' cwd="$R7B" $C)" \
  "RUN a role may not install another role's profile → DENY" "runtime profile"
KERNEL_MANDATE_MANIFEST="$R7B/.claude/kernel-mandate.json" assert_deny "$H" \
  "$(payload tool_name=Bash command='kernel-mandate run --role=admin -- node x.js' cwd="$R7B" $C)" \
  "RUN the attached --role=X form → DENY" "runtime profile"
KERNEL_MANDATE_MANIFEST="$R7B/.claude/kernel-mandate.json" assert_deny "$H" \
  "$(payload tool_name=Bash command='npx kernel-mandate run --role admin -- node x.js' cwd="$R7B" $C)" \
  "RUN through npx → DENY" "runtime profile"
KERNEL_MANDATE_MANIFEST="$R7B/.claude/kernel-mandate.json" assert_allow "$H" \
  "$(payload tool_name=Bash command='kernel-mandate run --role composer -- node x.js' cwd="$R7B" $C)" \
  "RUN calibration: a role installing its OWN profile → ALLOW"
KERNEL_MANDATE_MANIFEST="$R7B/.claude/kernel-mandate.json" assert_allow "$H" \
  "$(payload tool_name=Bash command='kernel-mandate run --role composer --dry-run -- node x.js' cwd="$R7B" $C)" \
