...
#
# for the manifest at
# schemas/kernel-mandate.fixtures/valid-registration-form-qa.json. This is
# the leak-proofing claim, demonstrated on a real task rather than a toy.

H="$HOOK_DIR/kernel-mandate-role-gate.sh"

section "kernel-mandate benchmark — registration-form automation"

BM=$(mktemp -d)
P="$BM/vue-test-app"
mkdir -p "$P/.claude" "$P/tests/e2e" "$P/tests/data" "$P/docs/acceptance" "$P/src"
export KERNEL_MANDATE_STATE_DIR="$BM/state"
...
cp "$HOOK_DIR/../schemas/kernel-mandate.fixtures/valid-registration-form-qa.json" "$P/.claude/kernel-mandate.json"
...
assert_deny "$H" "$(payload tool_name=Agent description='deployer-x: ship it' prompt='<<kernel-mandate-role: deployer>> deploy' cwd="$P")" \
...
assert_deny "$H" "$(payload tool_name=Write file_path="$P/.claude/kernel-mandate.json" content='{}' cwd="$P" $J)" \
  "judge rewrites the manifest to widen itself → DENY (root of trust)" "modify the kernel mandate itself"
