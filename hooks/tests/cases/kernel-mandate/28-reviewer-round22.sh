...
#       writing. The axis is a `case "$KM_TOOL"` with two arms —
...
section "kernel-mandate reviewer round-22 regressions"

R22=$(mktemp -d)
P="$R22/proj"
mkdir -p "$P/.claude/kernel-mandate.state/agents" "$P/.claude/hooks"
export KERNEL_MANDATE_MANIFEST="$P/.claude/kernel-mandate.json"
unset KERNEL_MANDATE_STATE_DIR   # exercise the DEFAULT state-dir location
...
JSON
printf 'config-keeper\n' > "$P/.claude/kernel-mandate.state/agents/ck"
...
assert_deny "$H" "$(mcpw "$P/.claude/kernel-mandate.json")" \
  "F1 a mapped MCP write may not rewrite the manifest → DENY" "modify the kernel mandate itself"
assert_deny "$H" "$(mcpw "$P/.claude/kernel-mandate.state/agents/attacker")" \
  "F1 nor forge a role binding in the state dir → DENY" "modify the kernel mandate itself"
assert_deny "$H" "$(mcpw "$P/.claude/settings.json")" \
  "F1 nor rewrite the hook registration → DENY" "modify the kernel mandate itself"
assert_deny "$H" "$(mcpw "$P/.claude/settings.local.json")" \
  "F1 nor its local override → DENY" "modify the kernel mandate itself"
assert_deny "$H" "$(mcpw "$P/.claude/hooks/evil.sh")" \
  "F1 nor plant a hook beside the kernel → DENY" "modify the kernel mandate itself"
assert_deny "$H" "$(mcpw "$P/node_modules/@civitas-cerebrum/kernel-mandate/hooks/kernel-mandate-role-gate.sh")" \
  "F1 nor truncate the installed kernel → DENY" "modify the kernel mandate itself"

# The core tools were always protected; pin that the three channels now
# agree rather than merely that the new one works.
assert_deny "$H" "$(wr "$P/.claude/kernel-mandate.json")" \
  "F1 the Write tool, as before → DENY" "modify the kernel mandate itself"
assert_deny "$H" "$(wr "$P/.claude/hooks")" \
  "F1 the bare .claude/hooks entry the Bash copy had DRIFTED without" \
  "modify the kernel mandate itself"
...
assert_allow "$H" "$(mcpr "$P/.claude/kernel-mandate.json")" \
...
export KERNEL_MANDATE_STATE_DIR="$R22/fstate"
export KERNEL_MANDATE_MANIFEST="$FP/.claude/kernel-mandate.json"
...
assert_allow "$H" "$(fp Task '{"description":"fetcher: go","prompt":"<<kernel-mandate-role: fetcher>>\ndo it"}')" \
  "self-probe calibration: a properly tagged Task → ALLOW"
assert_allow "$H" "$(fp Agent '{"description":"fetcher: go","prompt":"<<kernel-mandate-role: fetcher>>\ndo it"}')" \
