...
section "kernel-mandate reviewer round-2 regressions"
...
export KERNEL_MANDATE_STATE_DIR="$R2/state"
...
assert_deny "$H" "$(payload tool_name=Bash command='echo x > .claude/settings.json' cwd="$P" $C)" \
  "FP but an actual write TO .claude → DENY (self-protection intact)" "kernel mandate itself"
