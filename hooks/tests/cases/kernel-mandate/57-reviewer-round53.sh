...
section "kernel-mandate reviewer round-53 regressions"
...
export KERNEL_MANDATE_STATE_DIR="$R53/state"
...
assert_deny "$H" "$(b 'curl -s http://localhost:4173/ -o .claude/kernel-mandate.json')" \
  "R53 calibration: a real file target is still refused → DENY" "kernel mandate itself"
