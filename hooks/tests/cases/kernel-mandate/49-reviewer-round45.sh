...
#     uniq docs/acceptance/registration.md .claude/kernel-mandate.json
#
...
# had — `head -5 .claude/kernel-mandate.json` and `tail -n 3 …` were refused
...
section "kernel-mandate reviewer round-45 regressions"

R45=$(mktemp -d)
P="$R45/proj"
mkdir -p "$P/.claude" "$P/docs" "$P/tests/e2e"
printf 'AC-1: the form validates.\n' > "$P/docs/acceptance.md"
export KERNEL_MANDATE_STATE_DIR="$R45/state"
...
assert_deny "$H" "$(b 'uniq docs/acceptance.md .claude/kernel-mandate.json')" \
  "R45 uniq's second POSITIONAL operand is a write → DENY" "kernel mandate itself"
assert_deny "$H" "$(b 'uniq docs/acceptance.md .claude/settings.json')" \
  "R45 ...including a file that does not exist yet → DENY" "kernel mandate itself"
assert_deny "$H" "$(b 'uniq docs/acceptance.md .claude/kernel-mandate.state/agents/forged')" \
  "R45 ...and a forged role binding in the state dir → DENY" "kernel mandate itself"
assert_deny "$H" "$(b 'uniq tests/e2e/evil.json .claude/kernel-mandate.json' comp)" \
  "R45 the composer's two-step takeover, at step two → DENY" "kernel mandate itself"
...
assert_deny "$H" "$(b 'somefilter docs/acceptance.md .claude/kernel-mandate.json')" \
  "R45 an entirely unmodelled verb → DENY" "kernel mandate itself"
assert_deny "$H" "$(b 'tee .claude/kernel-mandate.json')" \
  "R45 calibration: tee, which was already refused → DENY" "kernel mandate itself"
assert_deny "$H" "$(b 'sort -o .claude/kernel-mandate.json docs/acceptance.md')" \
  "R45 calibration: sort -o, the flag form → DENY" "kernel mandate itself"
assert_deny "$H" "$(b 'curl -D .claude/kernel-mandate.json http://x')" \
  "R45 calibration: round 15's detached curl flag → DENY" "kernel mandate itself"
assert_deny "$H" "$(b 'curl -D.claude/kernel-mandate.json http://x')" \
  "R45 calibration: ...and its attached spelling → DENY" "kernel mandate itself"
...
for spec in \
  'cat .claude/kernel-mandate.json|cat' \
  'head -5 .claude/kernel-mandate.json|head with a numeric flag (was a FALSE POSITIVE)' \
  'tail -n 3 .claude/kernel-mandate.json|tail with a detached numeric flag (ditto)' \
  'jq -r .name .claude/kernel-mandate.json|jq' \
  'grep -c roles .claude/kernel-mandate.json|grep' \
  'wc -l .claude/kernel-mandate.json|wc' \
  'diff .claude/kernel-mandate.json docs/acceptance.md|diff, both operands read' \
  'grep --file=.claude/kernel-mandate.json docs/acceptance.md|a reader FLAG, attached' \
  'grep --file .claude/kernel-mandate.json docs/acceptance.md|a reader FLAG, detached' ; do
  cmd="${spec%%|*}"; label="${spec##*|}"
...
assert_deny "$H" "$(b 'echo x > .claude/settings.json' comp)" \
  "R45 calibration: ...but the redirect itself is still refused → DENY" "kernel mandate itself"
...
.claude/kernel-mandate.json')" \
  "R45 a backslash line-continuation is joined before splitting → DENY" "kernel mandate itself"
assert_deny "$H" "$(b 'uniq \
docs/acceptance.md \
.claude/kernel-mandate.json')" \
  "R45 ...however many of them there are → DENY" "kernel mandate itself"
assert_deny "$H" "$(b '.claude/kernel-mandate.json')" \
  "R45 ...and a protected path as the COMMAND WORD is not a free pass → DENY" "kernel mandate itself"
...
# "attempted to modify the kernel mandate itself". The verdict happened to
