...
#
#     KM_ROOT=$(git rev-parse --show-toplevel || echo "$KM_CWD")
#     KM_MANIFEST="$KM_ROOT/.claude/kernel-mandate.json"
#     [ -f "$KM_MANIFEST" ] || return 1     ->  exit 0  ->  ALLOW
#
...
# and the directory holding the first `.claude/kernel-mandate.json` IS the
...
section "kernel-mandate reviewer round-27 regressions"

R27=$(mktemp -d)
export KERNEL_MANDATE_STATE_DIR="$R27/state"
...
mk_project() {  # mk_project <dir>
  ...
  printf '%s\n' "$MANIFEST_BODY" > "$1/.claude/kernel-mandate.json"
...
# KERNEL_MANDATE_MANIFEST would pin the answer and hide the bug, which is
...
if [ -d "$MONO/.git" ]; then
  ...
  assert_deny "$H" "$(wp "$QA/.claude/kernel-mandate.json" "$QA")" \
    "A1 the reviewer may not overwrite the manifest → DENY" "kernel mandate"
...
}' > "$NEST/.claude/kernel-mandate.json"
printf 'x\n' > "$NEST/anything.txt"
...
printf 'reviewer\n' > "$KERNEL_MANDATE_STATE_DIR/agents/$(printf 'a_b')"
