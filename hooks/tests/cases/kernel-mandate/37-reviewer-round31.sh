...
# that `kernel-mandate init --template feature-dev` writes into a new
...
#
#   B  `kernel-mandate validate` certified it with no warning at all. The
#      write-then-execute check asked whether SOME command pattern
#      mentions `kernel-mandate run`; the implementer had a profiled test
...
#      dangerous one.
#
#   C  and it cannot be contained by routing it through `kernel-mandate run`
...
section "kernel-mandate reviewer round-31 regressions"

R31=$(mktemp -d)
P="$R31/proj"
mkdir -p "$P/.claude" "$P/src"
printf 'SMTP_PASSWORD=hunter2\n' > "$P/.env"
export KERNEL_MANDATE_STATE_DIR="$R31/state"
...
JSON
printf 'coder\n' > "$KERNEL_MANDATE_STATE_DIR/agents/coder"
