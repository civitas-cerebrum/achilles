...
#       and it cancels the runtime profile `kernel-mandate run` exists to
#       install.
#
...
section "kernel-mandate reviewer round-23 regressions"

R23=$(mktemp -d)
P="$R23/proj"
mkdir -p "$P/.claude" "$P/tests/e2e"
printf 'SMTP_PASSWORD=hunter2\n' > "$P/.env"
export KERNEL_MANDATE_STATE_DIR="$R23/state"
...
printf 'browsers\n' > "$KERNEL_MANDATE_STATE_DIR/agents/browsers"
