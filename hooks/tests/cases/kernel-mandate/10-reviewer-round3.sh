...
section "kernel-mandate reviewer round-3 regressions"
...
export KERNEL_MANDATE_STATE_DIR="$R3/state"
...
assert_deny "$H" "$(payload tool_name=Bash command='find tests -fprintf .claude/kernel-mandate.json %p' cwd="$P" $C)" \
  "N3 an output flag aimed at the manifest → DENY (self-protection, not scope)" "kernel mandate itself"
assert_deny "$H" "$(payload tool_name=Bash command='sort -o .claude/kernel-mandate.json tests/e2e/existing.spec.ts' cwd="$P" $C)" \
  "N3 'sort -o' aimed at the manifest → DENY" "kernel mandate itself"
