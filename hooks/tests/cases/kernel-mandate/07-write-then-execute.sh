...
section "kernel-mandate write-then-execute containment"

WX=$(mktemp -d)
P="$WX/proj"
mkdir -p "$P/.claude" "$P/tests/e2e" "$P/tests/fixtures" "$P/docs" "$P/scripts"
export KERNEL_MANDATE_STATE_DIR="$WX/state"
...
printf 'fixture-author\n' > "$KERNEL_MANDATE_STATE_DIR/agents/fa"
printf 'scribe\n' > "$KERNEL_MANDATE_STATE_DIR/agents/sc"
