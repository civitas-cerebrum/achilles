...
#       it walk up and stop at the first `.claude/kernel-mandate.json`, on
...
#       `tests/e2e/.claude/kernel-mandate.json` matched neither. The bench's
...
section "kernel-mandate reviewer round-44 regressions"
...
export KERNEL_MANDATE_STATE_DIR="$R44/state"
...
# no KERNEL_MANDATE_MANIFEST seam. The state dir stays explicit.
...
assert_deny "$H" "$(wp "tests/e2e/.claude/kernel-mandate.json" "$PWN" Edit)" \
  "R44 ...and the Edit spelling of it → DENY" "kernel mandate itself"
assert_deny "$H" "$(wp "tests/e2e/sub/deep/.claude/kernel-mandate.json" "$PWN")" \
  "R44 ...at any depth → DENY" "kernel mandate itself"
assert_deny "$H" "$(wp "tests/e2e/.claude/settings.json" '{}')" \
  "R44 ...and the rest of a nested .claude, not just the manifest → DENY" "kernel mandate itself"
assert_deny "$H" "$(wp "tests/e2e/.claude/hooks/evil.sh" '#!/bin/sh')" \
  "R44 ...including a hook planted beside a nested config → DENY" "kernel mandate itself"
...
assert_deny "$H" "$(wp "tests/e2e/kernel-mandate.json" "$PWN")" \
  "R44 ...and the manifest filename outside a .claude dir → DENY" "kernel mandate itself"
# The channel that was already right, kept honest.
assert_deny "$H" "$(bp 'echo {} > tests/e2e/.claude/kernel-mandate.json')" \
  "R44 calibration: the Bash spelling was always refused → DENY" "kernel mandate itself"
...
assert_allow "$H" "$(wp "tests/e2e/notes.md" 'The manifest lives in .claude/kernel-mandate.json.')" \
...
printf 'not json' > "$P/tests/e2e/.claude/kernel-mandate.json"
...
printf 'x\n' > "$LONE/tests/a.txt"
printf 'not json' > "$LONE/.claude/kernel-mandate.json"
...
for spec in \
  ...
  "tests/e2e/.claude/Kernel-Mandate.State/agents/x|a forged binding in a mixed-case state dir" \
  ".CLAUDE/settings.local.json|and the ROOT list folds too" ; do
  path="${spec%%|*}"; label="${spec##*|}"
  assert_deny "$H" "$(wp "$path" 'x')" "R44 $label → DENY" "kernel mandate itself"
done
assert_deny "$H" "$(bp 'echo x > tests/e2e/.CLAUDE/settings.json')" \
  "R44 ...and the Bash channel folds case as well → DENY" "kernel mandate itself"
assert_deny "$H" "$(bp 'rm -rf tests/e2e/.CLAUDE')" \
  "R44 ...including a mutate verb against it → DENY" "kernel mandate itself"
